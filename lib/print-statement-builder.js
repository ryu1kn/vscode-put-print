...
class PrintStatementBuilder {
    ...
    build(printStatementConfig, selectedText) {
        const escapedSelectedText = this._replaceWithRules(selectedText, printStatementConfig.escapeRules);
        const replacePairs = this._getReplacePairs(selectedText, escapedSelectedText);
        return this._replaceWithRules(printStatementConfig.template, replacePairs);
    }
    ...
    _replaceWithRules(text, replaceRules) {
        const firstRule = replaceRules && replaceRules[0];
